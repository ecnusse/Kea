...
KEY_SearchEvent = "search"
KEY_SetTextAndSearchEvent = "set_text_and_search"
KEY_ExitEvent = "exit"
...
class ReInstallAppEvent(InputEvent):
    ...
    def send(self, device):
        ...
        device.send_documents(self.app)
        # if "anki" in self.package:
        #     device.mkdir("/storage/emulated/0/AnkiDroid/")
        #     device.push_file(os.path.join("Document","collection.anki2"), "/storage/emulated/0/AnkiDroid/")
...
class SearchEvent(InputEvent):
    ...
    def __init__(self, event_dict=None):
        super().__init__()
        self.event_type = KEY_SearchEvent
        self.time = time.time()
        if event_dict is not None:
            self.__dict__.update(event_dict)
...
class SetTextAndSearchEvent(InputEvent):
    """
    use the uiautomator2's send_keys and search method
    """

    def __init__(self, text, event_dict=None):
        super().__init__()
        self.event_type = KEY_SetTextAndSearchEvent
        self.text = text
        if event_dict is not None:
            self.__dict__.update(event_dict)
