...
class UTG(object):
    ...
    def add_transition(self, event, old_state, new_state):
        ...
        if (old_state.structure_str, new_state.structure_str) not in self.G2.edges():
            self.G2.add_edge(
                old_state.structure_str, new_state.structure_str, events={}
            )
        self.G2[old_state.structure_str][new_state.structure_str]["events"][
            event_str
    ...
    def __output_utg(self):
        ...
        for state_str in self.G.nodes():
            ...
            utg_node = {
                ...
                "title": state_desc,
                "content": "\n".join(
                    [package_name, activity_name, state.state_str, state.search_content]
                ),
            }
        ...
        for state_transition in self.G.edges():
            ...
            for event_str, event_info in sorted(
    ...
    def reachable_from_one_state_to_another(self, from_state:str, to_state:str):
        ...
        if to_state in nx.descendants(self.G, from_state):
            return True
        return False
    # get states on the shortest path to the target state
    ...
    def get_navigation_steps(self, from_state, to_state):
        ...
        try:
            ...
            state_strs = nx.shortest_path(
                G=self.G, source=from_state_str, target=to_state_str
            )
            if not isinstance(state_strs, list) or len(state_strs) < 2:
                self.logger.warning(
                    f"Error getting path from {from_state_str} to {to_state_str}"
                )
    ...
    def get_G2_nav_steps(self, from_state, to_state):
        ...
        try:
            nav_steps = []
            state_strs = nx.shortest_path(
                G=self.G2, source=from_state_str, target=to_state_str
            )
            if not isinstance(state_strs, list) or len(state_strs) < 2:
                return None
            start_state_str = state_strs[0]
    ...
    def find_activity_according_to_state_str(self, state_str):
        for node_state_str in self.G.nodes:
            if node_state_str == state_str:
                return self.G.nodes[node_state_str]["state"].activity_short_name

    def find_structure_str(self, state_str):
        for node_state_str in self.G.nodes:
            if node_state_str == state_str:
